...
- `backend-api/src/`: entrypoints and transport layer (`main.rs`, `lib.rs`, `routes/`, `dto/`, `use_cases/`, auth/config/observability).
...
- Multi-step orchestration (gate checks + multiple repo calls) lives in `use_cases/` functions. Route handlers only parse the request, call the use-case, publish side-effects, and map the outcome to an HTTP response.
...
- Repositories expose ONLY the shared mixin methods (`getOne`, `getMany`, `createOne`, `deleteOne`, `updateOne`). No custom named methods on repo interfaces. Repos are thin CRUD adapters that delegate to services.
- Named domain operations that do not map to standard CRUD (e.g., toggle reaction, mute channel) are implemented as use-cases in `features/<feature>/use_cases/`. Use-cases may depend on repos and/or services.
- BLoCs depend on repos, use-cases, and runtime services. BLoCs never depend on HTTP services directly. Exceptions: `AuthenticationBloc`, `SettingsBloc`.
- Domain models live in `shared/models/`, not in service files.
- Search is modeled as a filtered `getMany` using a `Query` object with an optional `Filter` field, not as a separate repo method.
